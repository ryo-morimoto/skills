...
- Correct when a one-off prompt suffices, when recurrence is anticipatory, or when the skill mechanism cannot deliver the behavior (always-on conduct, one-step wishes—see the never-fires early exit).
...
- Gate each part independently; emit one scorecard and one routing row per part.
...
- A part that fails trigger fitness in the under-firing direction may still ride inside a sibling part that was admitted, for the moments that sibling fires; route it as merge into the admitted sibling and note that coverage outside those moments is lost.
